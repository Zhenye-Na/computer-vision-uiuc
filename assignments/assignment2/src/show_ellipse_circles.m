function show_ellipse_circles(I, cx, cy, rad1, rad2, threshold, scale, k)
...
rad_1 = rad1(:,ones(size(theta)));
rad_2 = rad2(:,ones(size(theta)));
theta = theta(ones(size(cx1,1),1),:);
X = cx1+cos(theta).*rad_1;
Y = cy1+sin(theta).*rad_2;
